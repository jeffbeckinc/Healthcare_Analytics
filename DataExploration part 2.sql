----- Reviewing each table and see what for data transformations can be done, what questions we can answer with this data

--- Consolidating fields into fewer tables
...
--- After consolidating fields into on primary fact table, now we review the data to see if anything additional data cleaning can occur
--- First, check the impact of Nulls across the table
...
SELECT 
 ...
 -  SUM(CASE WHEN g.GDP_SpendOnHealthcare IS NULL THEN 1 
  ...
  FROM [Healthcare].[dbo].[AnnualCHEPerCapita] a
	...
	--- Review the missing Code and Continent values
  ...
  FROM [Healthcare].[dbo].[AnnualCHEPerCapita] a
	...
	-- Now check Nulls after adding missing continent values
	SELECT 
    COUNT(*) AS total_rows,
    SUM(CASE WHEN a.Entity IS NULL THEN 1 
        ELSE 0 END) AS missing_entity,
 ...
 -  SUM(CASE WHEN g.GDP_SpendOnHealthcare IS NULL THEN 1 
  ...
  FROM [Healthcare].[dbo].[AnnualCHEPerCapita] a
	...
	-- Table is probably better off removed and just used by itself, but will keep in dataset for time being
